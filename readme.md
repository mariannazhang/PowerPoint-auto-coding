...
- `SaveToExcel_end`: Calls the helper function `SaveToExcel`, which saves the scripting dictionary `data` to the Excel datasheet `data.xlsx` by:
  ...
  - Sets participant number based on target row.
...
- `inProgress_SaveToExcel`: Just like `SaveToExcel_end`, it calls the helper function `SaveToExcel`, except it keeps "in_progress" as "yes", which allows subsequent macros to continue writing on the same row. Should be used immediately before exiting an in progress slideshow. Can be used multiple times throughout a study.
...
Helper macros to reset target objects on slides (designed for resource allocation using `measure_allocation`):
- `reset_verticallyBtwnAnchors`: Resets target objects (objects whose name contains `target`) on a *manually specified* slide to be evenly distributed vertically (minus some headroom for the researcher's video thumbnail) between anchor objects (objects or groups of objects whose name contains `anchor`)
...
2. Open `stimuli.pptm` (`.pptm` means it's macro-enabled) in PowerPoint. Adapt the template slides to be your stimuli slides.
  - Unfortunately, only single shapes can be linked to macros. Groups of shapes cannot activate macros. You can link a single object in the group, draw a shape on top of all of them, or use an image editor to replace them all with a single image.
